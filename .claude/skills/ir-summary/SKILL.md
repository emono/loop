...
- `sources.md` のウォッチ対象＋Web検索で、**直近の決算短信・適時開示・IR資料**の候補を数件挙げる。
- 各候補に **一次ソース URL** と要約妙味（何が動いたか）を1行で添える。
...
### 5. 提出（01_review_queue へ・投稿しない）
- `${CLAUDE_PROJECT_DIR}/accounts/<account>/<media>/01_review_queue/<YYYY-MM-DD>-<slug>.md` に保存。
- 中身: 下書き本文（案）＋ 使った一次ソース ＋ コンプラチェック結果 ＋ 未確認点。
- **ここで停止**。ユーザーの承認を待つ。投稿・API 送信はしない。

### 6. record（state 遷移・人間指示で）
配信パイプラインは番号順の3 state（sortable）: **`01_review_queue` → `02_approved` → `03_posted`**。
- **承認された**（例「OK / 次の state へ」）→ ファイルを `01_review_queue/` から **`02_approved/`** へ move。`state/posted.md` に `state=approved` で追記（重複回避用）、`state/progress.txt` を更新。
- **実際に X 投稿した**と伝えられたら → ファイルを `02_approved/` から **`03_posted/`** へ move。ファイル冒頭と `state/posted.md` に **投稿URL**を記録（`state=posted`）。
